...
function lastQueryString(value) {
    if (Array.isArray(value)) {
        return value[value.length - 1];
    }
...
export function authenticationMiddlewareVariant(allowedSource) {
    return (req, res, next) => {
...
export function authenticationMiddleware(req, res, next, allowedSource) {
    ...
    if (providedApiKey && (!allowedSource || allowedSource === 'apikey')) {
        ...
        const allowedKeys = [configuredKey, ...configuredKeys].filter((k) => !!k);
